...
class ResultView: UIView {
    ...
    private lazy var vStaskView: UIStackView = {
    ...
    private lazy var hStaskView: UIStackView = {
        let stackView = UIStackView(arrangedSubviews: [
            totalBillView,
            UIView(),
            totalTipView
    ...
    func configure(result: Result) {
        let text = NSMutableAttributedString (
            string: result.amountPerPesson.currencyFormatted,
            attributes: [.font: ThemeFont.bold(ofSize: 48)])
        text.addAttributes([.font: ThemeFont.bold(ofSize: 24)], range: NSMakeRange(0, 1))
        amountPerPersonLabel.attributedText = text
        totalBillView.configure(amount: result.totalBill)
        totalTipView.configure(amount: result.tipBill)
    }
    
    private func layout() {
        backgroundColor = .white
        addSubview(vStaskView)
        vStaskView.snp.makeConstraints { make in
